...
export default function AIMentorPage() {
  ...
  return (
    <PageShell icon={Brain} title="AI Mentor" description="Assistente científico IA para treinamento de força baseado em evidência">
      <div className="glass-card flex flex-col" style={{ height: "calc(100vh - 220px)" }}>
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {messages.length === 0 && (
            <div className="text-center py-20 text-muted-foreground font-sans">
              ...
              <p className="text-lg font-display">Olá! Sou seu AI Mentor.</p>
              <p className="text-sm mt-2">Pergunte sobre treinamento de força, periodização, biomecânica, nutrição esportiva...</p>
            </div>
          )}
          {messages.map((m, i) => (
        ...
        <div className="p-4 border-t border-border">
          <form onSubmit={(e) => { e.preventDefault(); send(); }} className="flex gap-2">
            <Input value={input} onChange={(e) => setInput(e.target.value)} placeholder="Pergunte sobre treinamento de força..." className="bg-secondary border-border" disabled={loading} />
